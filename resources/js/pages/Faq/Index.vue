...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-6 w-full space-y-8">
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-3 w-full sm:w-auto">
                    ...
                    <Button
                        ...
                        class="bg-indigo-600 hover:bg-indigo-700 text-white h-10 px-4 font-black text-[10px] uppercase tracking-widest rounded-xl whitespace-nowrap"
                    >
            ...
            <!-- Add Form -->
            <div v-if="showAddForm && isSuperAdmin" class="bg-white dark:bg-slate-900 border border-indigo-200 dark:border-indigo-800 rounded-2xl p-6 space-y-4">
                <h3 class="text-[10px] font-black uppercase tracking-widest text-indigo-600">New FAQ Item</h3>
                ...
                <div class="space-y-1">
                    ...
                    <textarea
                        ...
                        rows="3"
                        placeholder="Answer..."
                        class="w-full rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-slate-950 px-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500/20 outline-none resize-none"
                    />
                </div>
                <div class="space-y-1">
                ...
                <div class="flex justify-end gap-2">
                    ...
                    <Button @click="submitAdd" :disabled="addForm.processing" class="bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-black uppercase tracking-widest">
                        Save
                    </Button>
                </div>
            </div>
            ...
            <div v-for="[category, items] in grouped" :key="category" class="space-y-2">
                <h2 class="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-600 dark:text-indigo-400 px-1 flex items-center gap-2">
                    <div class="w-4 h-px bg-indigo-300 dark:bg-indigo-700"></div>
                    {{ category }}
                </h2>
                ...
                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
                    <div v-for="(faq, idx) in items" :key="faq.id">
                        ...
                        <!-- Edit mode -->
                        <div v-if="editingId === faq.id" class="p-5 space-y-4 border-b border-slate-100 dark:border-slate-800 last:border-0 bg-indigo-50/40 dark:bg-indigo-900/10">
                            <div class="grid grid-cols-2 gap-4">
                                <div class="space-y-1">
                                    <label class="text-[10px] font-black uppercase tracking-widest text-slate-400">Category</label>
                            ...
                            <div class="flex justify-end gap-2">
                                ...
                                <Button size="sm" @click="saveEdit(faq)" :disabled="editForm.processing" class="h-8 bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-black uppercase tracking-widest">
                        ...
                        <div v-else :class="['border-b border-slate-100 dark:border-slate-800 last:border-0', idx % 2 === 1 ? 'bg-slate-50/50 dark:bg-slate-800/20' : '']">
                            ...
                            >
                                ...
                                <div class="flex items-center gap-2 shrink-0">
                                    ...
                                    <ChevronUp v-else class="w-4 h-4 text-indigo-500" />
                                </div>
                            </button>
