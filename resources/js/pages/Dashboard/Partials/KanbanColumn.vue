...
import { KANBAN_UI } from '@/lib/kanban-theme'; // 1. Import Theme

defineProps<{
    status: TaskStatus;
    tasks: ProjectDocument[];
    rowLabel: string;
    canCreateTask: (status: TaskStatus) => boolean; // 2. Receive the rule
}>();
...
<template>
    <div class="flex flex-col gap-4 min-h-[160px] bg-gray-50/40 rounded-[2rem] border border-dashed border-gray-200/60 p-4 relative">
        <draggable
            :model-value="tasks"
        ...
        <Button
            ...
            :class="[
                ...
                KANBAN_UI.subtleLabel // 4. Apply consistent typography
            ]"
        >
