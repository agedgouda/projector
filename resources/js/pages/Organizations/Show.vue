...
import { Building2, Globe, Mail } from 'lucide-vue-next';
...
const handleOrgSwitch = (id: string | number) => {
    router.get('/organizations', { org: id }, {
        preserveState: false, // Force reload to ensure cookies and data sync
        preserveScroll: true
    });
};
</script>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-8 max-w-5xl mx-auto w-full space-y-8">
            ...
            <div class="bg-indigo-600 rounded-2xl p-8 text-white shadow-xl shadow-indigo-500/20 relative overflow-hidden">
                <div class="relative z-10">
                    ...
                    <div class="flex flex-wrap gap-6 text-indigo-100 font-medium text-sm">
                        <span v-if="currentOrg.website" class="flex items-center gap-2"><Globe class="h-4 w-4" /> {{ currentOrg.website }}</span>
                        <span v-if="currentOrg.email" class="flex items-center gap-2"><Mail class="h-4 w-4" /> {{ currentOrg.email }}</span>
                    </div>
                </div>
            ...
            <div>
                ...
                <OrgUserTable :users="currentOrg.users" :show-admin-toggle="true" />
            </div>
        </div>
    </AppLayout>
</template>
