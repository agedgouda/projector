...
const failureStatuses = ['slack-connect-failed', 'slack-not-configured'];

// The inline status box below is easy to miss once the redirect lands back on the Configuration
// tab lower down the page — a failure also gets a toast so it's seen immediately.
onMounted(() => {
...
<template>
    ...
    >
            ...
            <TooltipProvider>
                <Tooltip :delay-duration="200">
                    ...
                    >
                        ...
                        <p>
                            Click "Connect Slack Workspace" below and approve
                            the install for this organization's workspace. This
                            lets Projector create tasks/events and import files
                            from bound Slack channels.
                        </p>
                        <p class="pt-1 font-semibold">
                            If the button doesn't work
                        </p>
                        <p>
                            An admin needs to set up a Slack app first
                            (<code>SLACK_CLIENT_ID</code>,
                            <code>SLACK_CLIENT_SECRET</code>,
                            <code>SLACK_SIGNING_SECRET</code> in
                            <code>.env</code>). See
                            <code>docs/slack-app-setup.md</code> in the repo for
                            step-by-step setup.
                        </p>
                    </TooltipContent>
                </Tooltip>
        ...
        <p class="text-sm text-muted-foreground">
            Connect this organization's Slack workspace to create tasks and
            events, and import files, from Slack.
        </p>
        ...
        >
            ...
            <p class="mt-1">
                An admin needs to create a Slack app and set
                <code>SLACK_CLIENT_ID</code>, <code>SLACK_CLIENT_SECRET</code>,
                and <code>SLACK_SIGNING_SECRET</code> in <code>.env</code>. See
                <code>docs/slack-app-setup.md</code> in the repo for
        ...
        <template v-else-if="slackConnected">
            ...
            >
                ...
                <p
                    ...
                    class="text-sm text-muted-foreground"
                >
                    No unbound channels found — the bot may need to be invited
                    to more channels, or every visible channel is already bound.
                </p>
                ...
                <div v-else class="flex flex-wrap items-center gap-3">
                    <Select
                        ...
                        @update:model-value="
                            (v) => (selectedChannelId = v as string)
                        "
                    >
