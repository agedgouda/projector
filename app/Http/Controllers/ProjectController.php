...
class ProjectController extends Controller
{
    ...
    /**
     * The calendar export's filters as the on-screen calendar sends them: sub-projects hidden
     * (?hidden_subprojects[]=...), tags (?tags[]=..., "none" for untagged), and the Tasks/Events
     * toggle (?show_tasks=&show_events=, each defaulting to true so a bare export link shows
     * everything).
     */
    private function calendarExportItems(Request $request, Project $project): \Illuminate\Support\Collection
    {
        return $this->calendars()->items(
            $project,
            array_map('strval', (array) $request->query('hidden_subprojects', [])),
            array_map('strval', (array) $request->query('tags', [])),
            $request->boolean('show_tasks', true),
            $request->boolean('show_events', true),
        );
    }

    private function calendars(): CalendarExportBuilder
    {
        return app(CalendarExportBuilder::class);
    }
     ...
     * either, see CalendarExportBuilder::items()) as a branded, calendar-styled PDF matching the
     * on-screen calendar, months stacked vertically one after another rather than paginated per
     * month.
     */
    public function exportCalendarPdf(Request $request, Project $project): \Illuminate\Http\Response
    {
        ...
        $contents = $this->calendars()->pdfContents($project, $this->calendarExportItems($request, $project));

        return response($contents, 200, [
            'Content-Type' => 'application/pdf',
            'Content-Disposition' => 'attachment; filename="'.$this->calendars()->filename($project, 'pdf').'"',
        ]);
    }

    /**
     * Export the project's entire calendar (visible sub-projects, tags, and Tasks/Events
     * toggle all respected, past months excluded — see CalendarExportBuilder::items()), as a
    ...
    public function exportCalendarCsv(Request $request, Project $project): StreamedResponse
    {
        ...
        $contents = $this->calendars()->csvContents($project, $this->calendarExportItems($request, $project));
        $filename = $this->calendars()->filename($project, 'csv');
    ...
    public function exportCalendarExcel(Request $request, Project $project): StreamedResponse
    {
        ...
        $contents = $this->calendars()->excelContents($project, $this->calendarExportItems($request, $project));

        return response()->streamDownload(function () use ($contents) {
            echo $contents;
        }, $this->calendars()->filename($project, 'xlsx'), [
